...
/*   Updated: 2024/09/24 14:55:17 by ymrabeti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (ac < 5 || ac > 6 || ft_atoi(av[1]) <= 0 || ft_atoi(av[2]) <= 0 || ft_atoi(av[3]) <= 0 || ft_atoi(av[4]) <= 0)
...
void ft_is_dead(t_philo *philo) 
{
    ...
    i = 0;
    while (1) {
        if (ft_actual_time() - philo[i].last_meal >= (unsigned long)philo->args->timetodie) {
            philo->args->dead = 1;
...
int main(int ac, char **av) {
    ...
    while (++i < prg->args->n_philos) {
        if (pthread_create(&thr[i], NULL, (void *)&philos_routine, (void *)&prg->philo[i]))
